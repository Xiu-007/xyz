  ...
  <div class="teacher-audit">
        ...
        <el-table-column label="操作" min-width="180" align="center">
          <template #default="scope">
            <el-button 
              ...
              :disabled="scope.row.status !== 'PENDING' && scope.row.status !== 'TEACHER_PENDING'"
            >
              审核
            </el-button>
    ...
    >
      <el-form :model="auditForm" label-width="100px">
        ...
        <el-form-item label="教师审批意见" required>
          <el-input
            type="textarea"
            v-model="auditForm.teacherOpinion"
            :rows="4"
            maxlength="200"
            show-word-limit
            placeholder="请填写审批意见"
        ...
        <el-form-item>
          <el-button type="primary" @click="submitAudit('APPROVED')">通过</el-button>
          <el-button type="danger" @click="submitAudit('REJECTED')">不通过</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
...
const auditStatus = [
  ...
  { value: 'TEACHER_PENDING', label: '需重新审核' },
  { value: 'TEACHER_REJECTED', label: '已打回' },
  { value: 'DEAN_PENDING', label: '已提交院长' }
]
...
const getStatusLabel = (status) => {
  if (status === 'APPROVED') {
    return '已提交院长'  // 将 APPROVED 状态显示为"已提交院长"
  }
...
const getStatusTagType = (status) => {
  switch (status) {
    ...
    case 'DEAN_PENDING': return 'success'
...
const teacherId = userStr ? JSON.parse(userStr).id : null

// 方法定义
const handleSearch = async () => {
  try {
    const response = await axios.get('/api/documents/teacher-audit', {
      params: {
...
const downloadDocument = async () => {
  ...
  try {
    ...
    link.href = url
    link.setAttribute('download', currentDocument.value.name)  // 使用 name 而不是 documentName
...
const submitAudit = async (status) => {
  if (!auditForm.value.teacherOpinion) {
  ...
  try {
    //根据当前文档状态决定下一步状态
    let nextStatus = status === 'APPROVED' ? 'DEAN_PENDING' : 'TEACHER_REJECTED'
    
    //如果是重新审核，追加新的审核意见
    let opinion = auditForm.value.teacherOpinion
    if (currentDocument.value.status === 'TEACHER_PENDING') {
      opinion = `重新审核意见: ${opinion}`
    }
    
    await axios.post(`/api/documents/${currentDocument.value.id}/teacher-audit`, {
      status: status === 'APPROVED' ? 'DEAN_PENDING' : 'TEACHER_REJECTED',
      teacherOpinion: auditForm.value.teacherOpinion
    })
