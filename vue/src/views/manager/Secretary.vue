<template>
  <div>
    ...
    <div class="card">
      ...
      <el-dialog title="教务秘书信息" width="40%" v-model="data.formVisible" :close-on-click-modal="false" destroy-on-close>
...
const load = () =>{
  request.get('/secretary/selectPage',{
    params:{
...
const handleDelete = (id) =>{
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/secretary/deleteById/' + id).then(res => {
      if (res.code === '200') {
        load()
...
const add = () =>{
  request.post('/secretary/add', data.form).then(res => {
    if  (res.code === '200') {
      ElMessage.success('操作成功')
...
const update = () => {
  request.put('/secretary/update', data.form).then(res => {
    if  (res.code === '200') {
      ElMessage.success('操作成功')
      data.formVisible = false
      load()
